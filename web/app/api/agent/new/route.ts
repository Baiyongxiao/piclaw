...
export async function POST(req: Request) {
  try {
    ...
    const { provider, modelId, mode, thinkingLevel, ...promptCommand } = command as { provider?: string; modelId?: string; mode?: "plan" | "act"; thinkingLevel?: string; [key: string]: unknown };

    const tempKey = `__new__${Date.now()}`;
    const { session, realSessionId } = await startRpcSession(tempKey, "", cwd, mode);
