import { type AgentMessage, uuidv7 } from "@piclaw/agent-core";
...
export class SessionManager {
	...
	setSessionFile(sessionFile: string): void {
		...
		if (this.store.sessionFileExists(this.sessionFile)) {
			this.fileEntries = this.store.loadEntries(this.sessionFile);

			// If file was empty, initialize it with a valid session header. If it was
			// non-empty but did not parse as a pi session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (!this._isEmptyFile(explicitPath)) {
	...
	/** Check if a session file is empty (0 bytes). */
	private _isEmptyFile(filePath: string): boolean {
		try {
	...
	private _rewriteFile(): void {
		...
		this.store.rewriteFile(this.sessionFile, this.fileEntries);
	}
	...
	usesDefaultSessionDir(): boolean {
		// Check if the session directory matches the default for this cwd
		const defaultDir = this.store.getDefaultSessionDir(this.cwd);
		return this.sessionDir === defaultDir;
	}
	...
	_persist(entry: SessionEntry): void {
		...
		if (!hasAssistant) {
			if (this.flushed) {
				this.store.appendToFile(this.sessionFile, entry);
			} else {
	...
	static continueRecent(cwd: string, sessionDir?: string): SessionManager {
		...
		return new SessionManager(store, cwd, dir, undefined, true);
	}
	...
	static forkFrom(
		...
		// Copy all non-header entries from source via the store
		for (const entry of sourceEntries) {
			if (entry.type !== "session") {
				store.appendToFile(newSessionFile, entry as SessionEntry);
			}
		}

		return new SessionManager(store, resolvedTargetCwd, dir, newSessionFile, true);
	}
	...
	static async list(cwd: string, sessionDir?: string, onProgress?: SessionListProgress): Promise<SessionInfo[]> {
		...
		const sessions = (await store.listSessions(dir, onProgress)).filter(
			(session) => session.cwd === "" || resolvePath(session.cwd) === resolvedCwd,
		);
