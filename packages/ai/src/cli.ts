...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @piclaw/ai <command> [provider]
...
Examples:
  npx @piclaw/ai login              # interactive provider selection
  npx @piclaw/ai login anthropic    # login to specific provider
  npx @piclaw/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @piclaw/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @piclaw/ai --help' for usage`);
	process.exit(1);
}
